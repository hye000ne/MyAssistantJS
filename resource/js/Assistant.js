...
class Assistant {
    ...
    static FIRST_RESPONSE = `안녕하세요! 궁금한 게 있으시면 말해보세요 😊\n 처음이라면 아래처럼 입력해보세요: \n 형식 / 날씨 또는  형식 / 맛집`;
    static DEFAULT_RESPONSE = '죄송해요, 이해하지 못했어요 😢';

    respond(command, keyword) {
        if (command == '나가기') {
            currentAssistant = null;
            return Assistant.FIRST_RESPONSE;
        }
    ...
    respond(command, keyword) {
        if (command == '형식') {
            switch (keyword) {
                ...
                default:
                    return `형식을 알고 싶은 주제를 알려주세요 😊 (예: 형식 / 날씨)`;
            }
        }
...
class WeatherAPIAssistant extends Assistant {
    async respond(command, keyword) {
        if (command !== '날씨' || !keyword) return super.respond(command);
